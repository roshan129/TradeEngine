...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Export stitched historical 5-minute features CSV for backtesting"
    )
    parser.add_argument(
        "--from-date",
        required=True,
        help="Start date in IST (YYYY-MM-DD)",
    )
    parser.add_argument(
        "--to-date",
        required=True,
        help="End date in IST (YYYY-MM-DD)",
    )
    parser.add_argument(
        "--chunk-days",
        type=int,
        default=28,
        help="Days per API request chunk (default: 28)",
    )
    parser.add_argument(
        "--symbol",
        default="",
    ...
    parser.add_argument(
        "--output",
        default="data/market_data/features/feature_history_output.csv",
        help="Output features CSV path (default: data/market_data/features/feature_history_output.csv)",
    )
    parser.add_argument(
        "--raw-output",
        default="",
        help="Optional raw stitched OHLCV CSV output path",
    )
    parser.add_argument(
        "--pause-seconds",
        type=float,
    ...
    while cursor <= to_date:
        ...
        candles = service.get_5min_candles_between(
            symbol=symbol,
            from_ist=from_ist,
            to_ist=to_ist,
        )
        ...
        print(
            f"Fetched {len(candles)} candles for {from_ist.date()} -> {to_ist.date()} "
    ...
    clean_df = MarketDataProcessor().full_clean_pipeline(raw_df)
    ...
    print(f"Saved features CSV: {args.output} (rows={len(features_df)})")
    if args.raw_output:
        print(f"Saved raw OHLCV CSV: {args.raw_output} (rows={len(clean_df)})")
