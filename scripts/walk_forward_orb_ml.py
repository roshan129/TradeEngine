...
from tradeengine.core.strategy import OpeningRangeBreakoutStrategy
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Monthly walk-forward: train ML, apply ORB + ML filter on next month."
    )
    parser.add_argument(
        "--features",
        required=True,
    ...
    parser.add_argument(
        "--output-dir",
        default="walk_forward_orb_results",
    ...
    parser.add_argument("--opening-end", default="09:20")
    parser.add_argument("--orb-sl-pct", type=float, default=0.0025)
    parser.add_argument("--orb-tp-pct", type=float, default=0.0025)
    parser.add_argument("--orb-prob-threshold", type=float, default=0.65)
    parser.add_argument("--orb-use-trend-filter", action="store_true")
    parser.add_argument("--orb-use-volatility-filter", action="store_true")
    parser.add_argument("--orb-min-atr-pct", type=float, default=0.001)
    parser.add_argument("--orb-min-bb-width", type=float, default=0.004)
    parser.add_argument(
        "--no-ml",
        action="store_true",
        help="Skip ML training/inference and run pure ORB (probability filter disabled)",
    )
...
def main() -> int:
    ...
    if not 0.0 <= args.orb_prob_threshold <= 1.0:
        raise ValueError("--orb-prob-threshold must be in [0, 1]")
    ...
    if not args.no_ml:
        ...
        if args.orb_prob_threshold > 0:
            print("[INFO] --no-ml enabled: forcing orb-prob-threshold to 0.0")
        args.orb_prob_threshold = 0.0
        trainer = None
    ...
    while train_end + args.test_months <= len(months):
        ...
        if not args.no_ml:
            ...
            trainer.save_model(
                ...
                str(model_path),
                metadata={
                    "model_version": "orb_walk_forward",
                    "fold": fold_idx,
        ...
        trades_path = out_dir / f"orb_fold_{fold_idx}_trades.csv"
        equity_path = out_dir / f"orb_fold_{fold_idx}_equity.csv"
        pred_path = out_dir / f"orb_fold_{fold_idx}_predictions.csv"
